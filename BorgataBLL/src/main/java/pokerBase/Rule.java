...
public class Rule {
	...
	public Rule(String ruleName){
		for (GameRuleDomainModel gmRule : GameRuleDAL.getRules()){
			if (gmRule.getRULENAME() == ruleName){
				...
				for (GameRuleCardsDomainModel gm : GameRuleCardsDAL.getCardsRules()){
					if (gm.getRULEID() == gmRule.getRULEID()){
						iCards.add(gm.getNBROFCARDS());
					}
				}
				iCardsToDraw = iCards.toArray(new Integer[iCards.size()]);
				break;
			}
		}
	}
	...
	public Integer[] getiCardsToDraw() {
		return iCardsToDraw;
	}

	public void setiCardsToDraw(Integer[] iCardsToDraw) {
		this.iCardsToDraw = iCardsToDraw;
	}
